...
export default function NestingView({
  ...
  }, [adicionaltxt, layer, qtdop]);

  const Verificar = () => {
    const inputs = [InputText, InputText2, InputText3].slice(0, qtdop).map(input => input.toUpperCase().trim());
    const answers = [txtCerto1, txtCerto2, txtCerto3].slice(0, qtdop).map(answer => answer.toUpperCase().trim());

    if (inputs.length !== answers.length) {
      console.log('ERRO: quantidade de input não corresponde a quantidade de respostas');
      return;
    }

    if (inputs.every((input, index) => input === answers[index])) {
