...
- **`seedExampleLessons()` tem guard próprio e independente**, escopado ao primeiro módulo de HTML (`lessons.count({ where: { module_id } })`). Isso é obrigatório: o guard de áreas já está satisfeito em qualquer banco que já foi seedado, então uma lição criada dentro de `seedAreasAndModules` nunca chegaria a rodar. Também não usa uma contagem global de `lessons` — senão qualquer lição criada em outro módulo, no futuro, impediria este seed de exemplo de rodar. Localiza a área por `name: 'HTML'` (coluna real, escrita pelo próprio seeder); continua valendo a regra de **nunca hardcodar um `id`**. O conteúdo é transcrito de `app/src/screens/classes/lessons/html/content/basic-html.ts` (o app é outro pacote, a API não depende dele) — com o inglês reescrito de verdade, já que o bloco `en` de lá está em português. Cria sempre nesta ordem fixa — HTML, depois CSS, depois JavaScript (nome direto em `areas.name`, sem tradução) —, cada uma com 4 módulos (`index` 0 a 3, esses sim traduzidos: "Módulo 1".."Módulo 4"/"Module 1".."Module 4"). **Os `id`s gerados não são previsíveis** (autoincrement do Postgres, nunca resetado por um `DELETE`) — nunca hardcode um `id` de área/módulo em código novo; use a posição no array de `GET /areas` (a primeira é sempre HTML) como no app.
