...
- **`GET /areas`** — lista todas as áreas (`{ id, name }`), ordenado por `id`. **Não recebe `locale`** (ver ponto sobre tradução abaixo). **`GET /modules?areaId=<id>&locale=pt|en`** — lista os módulos de uma área (`{ id, areaId, index, name, lessons }`), ordenados por `index`, cada um trazendo suas `lessons` (`{ id, index, name, activities }`) e cada lição trazendo suas `activities` (`{ id, index, type, content }`), tudo ordenado por `index` e já traduzido. Uma `areaId` sem módulos retorna `[]` (não é um 404 — é uma lista filtrada, igual `GET /users` sem resultados).
