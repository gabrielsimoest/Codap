...
- **`seedHtmlBasicLessons()` tem guard próprio e independente**, escopado ao primeiro módulo de HTML (`lessons.count({ where: { module_id } })`). Isso é obrigatório: o guard de áreas já está satisfeito em qualquer banco que já foi seedado, então uma lição criada dentro de `seedAreasAndModules` nunca chegaria a rodar. Também não usa uma contagem global de `lessons` — senão qualquer lição criada em outro módulo, no futuro, impediria este seed de rodar. Localiza a área por `name: 'HTML'` (coluna real, escrita pelo próprio seeder); continua valendo a regra de **nunca hardcodar um `id`**. Cria sempre nesta ordem fixa — HTML, depois CSS, depois JavaScript (nome direto em `areas.name`, sem tradução) —, cada uma com 4 módulos (`index` 0 a 3, esses sim traduzidos: "Módulo 1".."Módulo 4"/"Module 1".."Module 4"). **Os `id`s gerados não são previsíveis** (autoincrement do Postgres, nunca resetado por um `DELETE`) — nunca hardcode um `id` de área/módulo em código novo; use a posição no array de `GET /areas` (a primeira é sempre HTML) como no app.
- **O conteúdo semeado no primeiro módulo de HTML já é curricular real, não placeholder**: 8 lições ("O que é HTML?", "Estrutura de um documento HTML", "Texto e títulos", "Links", "Imagens", "Listas", "Atributos", "Div e Span"), cada uma com até 2 atividades `theory`, em pt e en. É **só teoria por enquanto** — a parte prática (`option`) desse módulo ainda está em validação editorial e entra depois, complementando estas mesmas lições (não substituindo). O conteúdo hardcoded equivalente do app (`app/src/screens/classes/lessons/html/content/basic-html.ts`) continua existindo em paralelo, acessível pelo gesto secreto (ver `app/CLAUDE.md`) — não é a fonte deste seed nem é mantido em sincronia com ele.
