...
> **Não confundir com o preview passivo, que já existe.** A aba "Web" do `CodeSection` já executa o bloco JavaScript de uma atividade teórica (`buildSandboxHtmlDocument`): DOM manipulado e `console.log` aparecem na própria página. Isso **não** é esta fase. A diferença é de quem escreve o código: no preview, o JavaScript é sempre autorado por quem escreve a lição e roda sem interação; nesta fase, é o **aluno** que escreve, o que traz correção automática, bridge RN↔WebView e o hardening listado em "Sandboxing" abaixo — nada disso existe hoje.
>
> Uma consequência prática, do lado do conteúdo: os módulos de JavaScript foram semeados **antes** do preview existir, então quase tudo lá continua `onlyCode: true` e não demonstra o próprio resultado. Só a tela 1 da lição "DOM" (módulo intermediário) foi revisada e ligada — é a referência de como as outras ficariam. Rever as demais é trabalho de conteúdo, tela a tela, independente desta fase: uma tela cujo JS não produz efeito visível nem `console.log` renderizaria um preview vazio (ver `api/CLAUDE.md`, "Conteúdo pedagógico").
