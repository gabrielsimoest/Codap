...
> **Custo já sendo pago hoje, no conteúdo.** Enquanto esta fase não existir, a aba "Web" do `CodeSection` monta o preview só com HTML + CSS (`buildPreviewHtml()` ignora blocos `JavaScript` de propósito), então **nenhuma tela teórica de JavaScript pode demonstrar o próprio resultado** — todas ficam `onlyCode: true`. Isso não é hipotético: as 24 telas do módulo intermediário de JavaScript já foram escritas assim, incluindo as de DOM e eventos, onde a proposta pedagógica aprovada pedia explicitamente uma demonstração visual (ver `api/CLAUDE.md`, "Conteúdo pedagógico"). É o argumento concreto a favor de priorizar a Fase 3 — o conteúdo de JS já está no banco esperando por ela. A saída improvisada (embutir um `<script>` num bloco `HTML` para a WebView executar) **não deve ser usada**: funcionaria, mas entrega execução de código sem nenhum dos controles listados em "Sandboxing" abaixo.
